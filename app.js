const diceConfig = [
    ...
    { id: 'pink', label: 'Pink', color: '#ec4899', text: '#fff' } // Changed Label
];
...
function showSagePopup() {
    ...
    overlay.innerHTML = `<div class="w-[85%] max-w-[320px] bg-white border-4 border-yellow-500 rounded-[40px] p-8 text-center shadow-2xl">
        <div class="flex flex-col items-center gap-6">
            <div class="w-24 h-24 bg-gradient-to-tr from-amber-400 to-yellow-600 rounded-full flex items-center justify-center text-white shadow-xl ring-4 ring-yellow-200"><svg class="w-14 h-14" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="4" d="M5 13l4 4L19 7"></path></svg></div>
            <div><h2 class="text-3xl font-black text-yellow-600 tracking-tighter mb-1">SAGE QUEST</h2><h3 class="text-xl font-black uppercase text-slate-400">COMPLETE</h3></div>
...
function renderGame() {
    ...
    let diceRowsHtml = (roundNum === 1) 
        ? `<div class="animate-fadeIn">${renderDiceRow(diceConfig[0], roundData)}</div><div class="mt-12 text-center animate-fadeIn px-4"><div class="expansion-gradient text-5xl font-black uppercase tracking-tight">Expansion Pack<br>Edition</div></div>`
        : diceConfig.map((dice, idx) => `<div class="animate-fadeIn" style="animation-delay: ${idx * 0.05}s">${renderDiceRow(dice, roundData)}</div>`).join('');

    let sageProgressBarHtml = '';
    let sageDiceRowHtml = '';

    if (roundNum >= 2) {
        if (sageGlobalStatus) {
            // THE NEW GOLD SAGE DICE CONTAINER
            sageDiceRowHtml = `
                <div onclick="setActiveInput('sage')" id="row-sage" class="dice-row p-5 rounded-2xl border-l-8 border-transparent cursor-pointer bg-yellow-500 text-black mb-3 scale-100 animate-fadeIn" style="transition: all 0.3s ease;">
                    <div class="flex justify-between items-center">
                        <span class="font-black uppercase tracking-widest text-lg">SAGE</span>
                        <span id="sage-sum" class="text-4xl font-black">0</span>
                    </div>
                    <div id="sage-values" class="flex flex-wrap gap-3 mt-3 min-h-[10px]"></div>
                </div>`;
        } else {
            sageProgressBarHtml = `<div id="sage-container" class="mb-6 p-4 bg-black/5 rounded-3xl border border-[var(--border-ui)] animate-fadeIn"><div class="flex justify-between items-end mb-2"><span class="text-[10px] font-black uppercase tracking-widest opacity-60">Sage Progress</span><span id="sage-status-text" class="text-xs font-black uppercase text-green-500">0/6 Used</span></div><div class="h-4 w-full bg-black/10 rounded-full overflow-hidden"><div id="sage-progress-fill" class="h-full bg-gradient-to-r from-green-500 to-emerald-400 transition-all duration-500" style="width: 0%"></div></div></div>`;
        }
    }
    ...
    app.innerHTML = `<div class="scroll-area" id="game-scroll">
        <div class="sticky top-0 bg-inherit backdrop-blur-md z-50 p-5 border-b border-[var(--border-ui)] flex justify-between items-center">
            ...
            <div class="flex items-center gap-6"><button onclick="changeRound(-1)" class="nav-btn ${roundNum === 1 ? 'disabled' : ''}">${leftChevron}</button><div class="text-center"><div class="text-xl font-black uppercase">Round ${roundNum}</div><div id="round-total-display" class="text-5xl font-black">0</div></div><button onclick="changeRound(1)" class="nav-btn ${roundNum === 10 ? 'disabled' : ''}">${rightChevron}</button></div><div class="w-10"></div>
        </div>
        <div class="p-4 pb-8">${prevRoundHtml}${sageProgressBarHtml}<div class="section-title animate-fadeIn"><h3>Dice Calculators</h3></div>
            <div class="space-y-3">${diceRowsHtml}
                ${sageDiceRowHtml}
                <div id="wild-section" class="wild-section-container animate-fadeIn ${roundNum < 2 ? 'hidden' : ''}">
                ...
                </div></div><div class="grand-total-footer animate-fadeIn"><span class="text-[10px] font-black uppercase opacity-50 block mb-1">Grand Total</span><span id="grand-total-box" class="text-5xl font-black">0</span></div>
        </div>
    </div>
...
function updateAllDisplays() {
    ...
    diceConfig.forEach(d => {
        ...
        if (valEl) valEl.innerHTML = vals.map((v, i) => `<span class="inline-flex items-center bg-black/10 px-5 py-3 rounded-2xl text-xl font-black border border-black/5 active:scale-95 transition-transform">${v} <button onclick="event.stopPropagation(); removeVal('${d.id}', ${i})" class="ml-4 w-8 h-8 flex items-center justify-center bg-black/20 rounded-full text-lg opacity-60">×</button></span>`).join('');
    });

    // Handle Gold Sage Container
    const sageVals = round.sage || [];
    const sageSum = sageVals.reduce((a,b) => a+b, 0);
    const sageSumEl = document.getElementById('sage-sum');
    if (sageSumEl) sageSumEl.textContent = sageSum;
    const sageValEl = document.getElementById('sage-values');
    if (sageValEl) sageValEl.innerHTML = sageVals.map((v, i) => `<span class="inline-flex items-center bg-black/20 px-5 py-3 rounded-2xl text-xl font-black border border-black/5 active:scale-95 transition-transform">${v} <button onclick="event.stopPropagation(); removeVal('sage', ${i})" class="ml-4 w-8 h-8 flex items-center justify-center bg-black/20 rounded-full text-lg opacity-60">×</button></span>`).join('');
...
function setActiveInput(id) {
    activeInputField = id;
    document.querySelectorAll('.dice-row').forEach(r => { 
...
// --- Standard Handlers (History/Settings/Input) ---
function resumeGame(i) { activeGame = games[i]; const modal = document.getElementById('action-modal'); if(modal) modal.remove(); renderGame(); }
function confirmDelete(index) { if(confirm("Permanently delete this game?")) { games.splice(index, 1); saveGame(); const modal = document.getElementById('action-modal'); if(modal) modal.remove(); showHome(); } }
...
function setWildTarget(idx, targetId) {
    ...
    if (card) {
        ...
        card.querySelectorAll('.wheel-item').forEach((item, i) => item.classList.toggle('selected', diceConfig.filter(d=>d.id!=='yellow')[i].id === targetId));
    }
...
function toggleSparkle() {
    ...
    if (btn) { btn.innerHTML = activeGame.rounds[activeGame.currentRound].blueHasSparkle ? 'Sparkle Activated ✨' : 'Add Sparkle?'; btn.className = `sparkle-btn-full ${activeGame.rounds[activeGame.currentRound].blueHasSparkle ? 'sparkle-on' : 'sparkle-off'}`; }
    updateAllDisplays(); saveGame();
}
...
function toggleMenu() {
    const ex = document.getElementById('menu-overlay'); if (ex) { ex.remove(); return; }
    const menu = document.createElement('div'); menu.id = 'menu-overlay'; menu.className = 'modal-overlay justify-end animate-fadeIn'; menu.onclick = (e) => { if(e.target === menu) menu.remove(); };
...
function startNewGame() { activeGame = { id: Date.now(), date: new Date().toLocaleString([], { month: 'short', day: 'numeric', hour: '2-digit', minute: '2-digit' }), currentRound: 0, rounds: Array(10).fill(null).map(() => ({ yellow: [], purple: [], blue: [], red: [], green: [], clear: [], pink: [], wild: [], sage: [], blueHasSparkle: false })) }; games.unshift(activeGame); saveGame(); renderGame(); }

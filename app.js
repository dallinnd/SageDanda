const diceConfig = [
    ...
    { id: 'sage', label: 'SAGE GOLD', color: '#eab308', text: '#000', isSage: true }
];
...
function showSagePopup() {
    // Permanent Unlock for current and all future rounds in this game
    for (let i = activeGame.currentRound; i < 10; i++) {
        activeGame.rounds[i].sageUnlocked = true;
    }
    saveGame();
    ...
    overlay.onclick = () => { overlay.remove(); renderGame(); };
...
function renderGame() {
    ...
    if (activeGame.currentRound > 0) {
        ...
        prevRoundInfoHtml = `
            <div class="animate-fadeIn">
                <div class="prev-round-box"><span>Prev Round Yellow Total</span><span class="text-xl">${(prevRound.yellow || []).reduce((a, b) => a + b, 0)}</span></div>
                <div class="prev-total-box"><span>Last Round Total Score</span><span class="text-xl">${calculateRoundTotal(prevRound)}</span></div>
            </div>`;
    }

    let sageUIHtml = '';
    if (roundNum >= 2) {
        if (roundData.sageUnlocked) {
            sageUIHtml = `<div class="sage-completed-banner mb-6 animate-fadeIn">✨ Sage Quest Completed ✨</div>`;
        } else {
            sageUIHtml = `
    ...
    if (roundNum === 1) {
        diceRowsHtml = `<div class="animate-fadeIn">${renderDiceRow(diceConfig[0], roundData)}</div>
                        <div class="mt-12 text-center animate-fadeIn px-4"><div class="expansion-gradient text-5xl font-black uppercase">Expansion Pack<br>Edition</div></div>`;
    } else {
        diceRowsHtml = diceConfig
            .filter(d => d.id !== 'sage' || roundData.sageUnlocked)
            .map((d, i) => `<div class="animate-fadeIn" style="animation-delay: ${i * 0.05}s">${renderDiceRow(d, roundData)}</div>`).join('');
    }
    ...
    app.innerHTML = `
        <div class="scroll-area" id="game-scroll">
            <div class="sticky top-0 bg-inherit backdrop-blur-md z-50 p-5 border-b border-[var(--border-ui)] flex justify-between items-center">
                ...
                <div class="flex items-center gap-6">
                    <button onclick="changeRound(-1)" class="nav-btn ${roundNum === 1 ? 'disabled' : ''}"><svg class="w-6 h-6" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="3" d="M15 19l-7-7 7-7"></path></svg></button>
                    ...
                    <button onclick="changeRound(1)" class="nav-btn ${roundNum === 10 ? 'disabled' : ''}"><svg class="w-6 h-6" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="3" d="M9 5l7 7-7 7"></path></svg></button>
                </div>
                <div class="w-10"></div>
            </div>
            <div class="p-4 pb-8">
                ${prevRoundInfoHtml}
                ${sageUIHtml}
                <div class="section-title"><h3>Dice Calculators</h3></div>
                <div class="space-y-3">
                    ${diceRowsHtml}
        ...
        <div id="keypad-container" class="keypad-area p-4 flex flex-col">
            ...
            <div class="grid grid-cols-4 gap-2 flex-1">
                ${[1,2,3].map(n => `<button onclick="kpInput('${n}')" class="kp-btn bg-black/5 text-3xl font-black">${n}</button>`).join('')}
                <button id="add-btn" onclick="kpEnter()" class="kp-btn bg-green-600 text-white row-span-4 h-full text-lg font-black">ADD</button>
                ${[4,5,6].map(n => `<button onclick="kpInput('${n}')" class="kp-btn bg-black/5 text-3xl font-black">${n}</button>`).join('')}
                ${[7,8,9].map(n => `<button onclick="kpInput('${n}')" class="kp-btn bg-black/5 text-3xl font-black">${n}</button>`).join('')}
                <button onclick="kpClear()" class="kp-btn bg-black/5 text-lg font-black">CLR</button>
                <button onclick="kpInput('0')" class="kp-btn bg-black/5 text-3xl font-black">0</button>
                <button onclick="kpToggleNeg()" class="kp-btn bg-black/5 text-2xl font-black">+/-</button>
            </div>
        </div>`;
    updateAllDisplays();
}
...
function renderWildCardHtml(w, idx) {
    ...
    return `<div onclick="setActiveWildInput(${idx})" id="wild-card-${idx}" class="wild-card ${activeInputField === 'wild-'+idx ? 'active-input' : ''}" style="border-left: 8px solid ${color}">
        ...
        <div class="color-picker-wheel">${diceConfig.filter(d => d.id !== 'yellow' && d.id !== 'sage').map(d => `<div onclick="event.stopPropagation(); setWildTarget(${idx}, '${d.id}')" class="wheel-item ${w.target === d.id ? 'selected' : ''}" style="background-color: ${d.color}"></div>`).join('')}</div>
    </div>`;
}
...
function calculateSageProgress(round) {
    ...
    diceConfig.filter(d => d.id !== 'sage').forEach(d => { if (round[d.id] && round[d.id].length > 0) usedColors.add(d.id); });
...
function updateAllDisplays() {
    ...
    if (!round.sageUnlocked) {
        ...
        if (sageFill) {
            ...
            sageFill.className = sage.count >= 6 ? "h-full bg-gradient-to-r from-amber-300 via-yellow-500 to-amber-600 transition-all duration-500" : "h-full bg-gradient-to-r from-green-500 to-emerald-400 transition-all duration-500";
        }
    }
    ...
    (round.wild || []).forEach((w, i) => { 
        ...
        const d = document.querySelectorAll('.wild-val-display')[i]; if(d) d.textContent = w.value || 0;
    });
    ...
    diceConfig.forEach(d => {
        ...
        const sumEl = document.getElementById(`${d.id}-sum`); if(sumEl) sumEl.textContent = score;
        const valEl = document.getElementById(`${d.id}-values`);
        if (valEl) {
            valEl.innerHTML = vals.map((v, idx) => `<span class="inline-flex items-center bg-black/10 px-5 py-3 rounded-2xl text-xl font-black shadow-sm border border-black/5 active:scale-95 transition-transform">${v}<button onclick="event.stopPropagation(); removeVal('${d.id}', ${idx})" class="ml-4 w-8 h-8 flex items-center justify-center bg-black/20 rounded-full text-lg opacity-60 active:bg-red-500 active:text-white transition-colors">×</button></span>`).join('');
        }
    });

    const rtEl = document.getElementById('round-total-display'); if(rtEl) rtEl.textContent = calculateRoundTotal(round);
    const gtEl = document.getElementById('grand-total-box'); if(gtEl) gtEl.textContent = calculateGrandTotal(activeGame);
}
...
function setActiveInput(id) {
    ...
    diceConfig.forEach(d => { const r = document.getElementById(`row-${d.id}`); if (r) { r.style.backgroundColor = ""; r.style.color = ""; r.classList.remove('active-sage'); } });
    const row = document.getElementById(`row-${id}`);
    if (row) { if (id === 'sage') row.classList.add('active-sage'); else { row.style.backgroundColor = config.color; row.style.color = config.text; } }
...
function changeRound(s) { 
    ...
    if (s === 1 && sage.count >= 6 && !rd.sageUnlocked) { showSagePopup(); return; }
...
function setWildTarget(idx, targetId) {
    ...
    saveGame(); renderGame(); setActiveWildInput(idx);
}
...
function toggleMenu() {
    ...
    const menu = document.createElement('div'); menu.id = 'menu-overlay'; menu.className = 'modal-overlay justify-end animate-fadeIn';
...
function clearHistory() { if (confirm("Delete ALL games?")) { games = []; activeGame = null; saveGame(); showHome(); } }
...
function startNewGame() { 
    activeGame = { id: Date.now(), date: new Date().toLocaleString([], { month: 'short', day: 'numeric', hour: '2-digit', minute: '2-digit' }), currentRound: 0, 
        rounds: Array(10).fill(null).map(() => ({ yellow: [], purple: [], blue: [], red: [], green: [], clear: [], pink: [], sage: [], wild: [], blueHasSparkle: false, sageUnlocked: false })) 
    }; 

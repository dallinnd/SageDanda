...
const diceConfig = [
    ...
    { id: 'pink', label: 'Pink', color: '#ec4899', text: '#fff' } // Label changed to Pink
];
...
function calculateRoundTotal(round) {
    ...
    // Add new Sage row totals if they exist
    if (round.sage) {
        total += round.sage.reduce((a, b) => a + b, 0);
    }
    return total;
}
...
// Checks if Sage was finished in this round OR any previous round
...
function renderGame() {
    ...
    if (activeGame.currentRound > 0) {
        ...
        prevHtml = `<div class="animate-fadeIn"><div class="prev-round-box"><span>Prev Round Yellow Total</span><span class="text-xl">${(pr.yellow || []).reduce((a,b)=>a+b,0)}</span></div><div class="prev-total-box"><span>Last Round Total Score</span><span class="text-xl">${calculateRoundTotal(pr)}</span></div></div>`;
    }
    ...
    // Logic to switch between Progress Bar and Gold Sage Row
    let sageBarHtml = '';
    let sageDiceRowHtml = '';
    ...
    if (roundNum >= 2) {
        if (sageGlobalStatus) {
            // Show Gold Sage Row
            sageDiceRowHtml = `
            <div onclick="setActiveInput('sage')" id="row-sage" class="dice-row p-5 rounded-2xl border-l-8 border-transparent cursor-pointer bg-yellow-500 text-black mb-3 animate-fadeIn">
    ...
    app.innerHTML = `
        <div class="scroll-area" id="game-scroll">
            ...
            <div class="p-4 pb-8">${prevHtml}${sageBarHtml}<div class="section-title"><h3>Dice Calculators</h3></div>
                <div class="space-y-3">
                    ${diceHtml}
                    ${sageDiceRowHtml}
...
// 7. Display Updates (No full re-render)
function updateAllDisplays() {
    const rd = activeGame.rounds[activeGame.currentRound];
    if (!rd) return;

    // Update Sage Progress Bar (if it exists)
    ...
    if (sText) {
        sText.textContent = `${sageProg.count}/6 Used${sageProg.count >= 6 ? ' - SAGE! ✨' : ''}`;
        sText.className = `text-xs font-black uppercase ${sageProg.count >= 6 ? 'text-yellow-500' : 'text-green-500'}`;
    }
    if (sFill) {
        sFill.style.width = `${sageProg.percentage}%`;
        sFill.className = `h-full transition-all duration-500 ${sageProg.count >= 6 ? 'bg-gradient-to-r from-amber-300 via-yellow-500 to-amber-600' : 'bg-gradient-to-r from-green-500 to-emerald-400'}`;
    }

    // Calculate Wild Bonuses
    const wildBonuses = {};
    (rd.wild || []).forEach(w => { if(w) wildBonuses[w.target] = (wildBonuses[w.target] || 0) + (w.value || 0); });

    // Update Standard Dice Rows
    diceConfig.forEach(d => {
        const vals = rd[d.id] || [];
        let base = (vals.reduce((a, b) => a + b, 0)) + (wildBonuses[d.id] || 0);
        let score = base;
    ...
    // Update Gold Sage Row (if it exists)
    const sageVals = rd.sage || [];
    const sageSumEl = document.getElementById('sage-sum');
    if(sageSumEl) sageSumEl.textContent = sageVals.reduce((a,b)=>a+b,0);
    const sageValEl = document.getElementById('sage-values');
    if(sageValEl) {
         sageValEl.innerHTML = sageVals.map((v, i) => `
            <span class="inline-flex items-center bg-black/20 px-5 py-3 rounded-2xl text-xl font-black shadow-sm border border-black/5 active:scale-95 transition-transform">
                ${v} <button onclick="event.stopPropagation(); removeVal('sage', ${i})" class="ml-4 w-8 h-8 flex items-center justify-center bg-black/20 rounded-full text-lg opacity-60 active:bg-red-500 active:text-white transition-colors">×</button>
            </span>`).join('');
    }
...
function setActiveInput(id) {
    ...
    if (row) {
        if (id === 'sage') {
            // Gold Row Active State (Black bg, Gold text)
            row.style.backgroundColor = "#000";
            row.style.color = "#fbbf24";
        } else {
            // Standard Dice Row Active State
...
function kpEnter() {
    ...
    if (activeInputField.startsWith('wild-')) {
        ...
        // Handles standard dice AND the new 'sage' row
        if (!rd[activeInputField]) rd[activeInputField] = [];
...
function startNewGame() {
    // Initialize new game with 'sage' array in every round
    activeGame = { id: Date.now(), date: new Date().toLocaleString([], { month: 'short', day: 'numeric', hour: '2-digit', minute: '2-digit' }), currentRound: 0, rounds: Array(10).fill(null).map(() => ({ yellow: [], purple: [], blue: [], red: [], green: [], clear: [], pink: [], wild: [], sage: [], blueHasSparkle: false })) };

...
function showSplash() {
    app.innerHTML = `
        <div class="h-full flex flex-col items-center justify-center bg-[#0f172a] cursor-pointer" onclick="showHome()">
...
function renderGame() {
    ...
    const sageGlobalStatus = isSageAlreadyComplete();

    // Sage Section UI
    let sageHtml = '';
    if (roundNum >= 2) {
        if (sageGlobalStatus) {
            sageHtml = `<div class="mb-6 p-6 bg-yellow-500/10 border-2 border-yellow-500/30 rounded-3xl flex items-center justify-center animate-fadeIn"><span class="text-yellow-600 font-black text-xl italic underline decoration-yellow-300">★ SAGE QUEST COMPLETE ★</span></div>`;
        } else {
            sageHtml = `<div id="sage-container" class="mb-6 p-4 bg-black/5 rounded-3xl border border-[var(--border-ui)] animate-fadeIn">
                ...
                <div class="h-4 w-full bg-black/10 rounded-full overflow-hidden"><div id="sage-progress-fill" class="h-full transition-all duration-500" style="width: 0%"></div></div>
            </div>`;
        }
    }
    ...
    if (activeGame.currentRound > 0) {
        ...
        prevHtml = `<div class="animate-fadeIn"><div class="prev-round-box"><span>Prev Yellow Total</span><span class="text-xl">${(pr.yellow || []).reduce((a,b)=>a+b,0)}</span></div><div class="prev-total-box"><span>Last Round Score</span><span class="text-xl">${calculateRoundTotal(pr)}</span></div></div>`;
    }
    ...
    app.innerHTML = `
        <div class="scroll-area" id="game-scroll">
            ...
            <div class="p-4 pb-8">${prevHtml}${sageHtml}<div class="section-title"><h3>Dice Calculators</h3></div><div class="space-y-3">${diceHtml}
    ...
    diceConfig.forEach(d => {
        ...
        if (valEl) {
            valEl.innerHTML = vals.map((v, i) => `
                <span class="inline-flex items-center bg-black/10 px-5 py-3 rounded-2xl text-xl font-black shadow-sm border border-black/5">
                    ${v} <button onclick="event.stopPropagation(); removeVal('${d.id}', ${i})" class="ml-4 w-8 h-8 flex items-center justify-center bg-black/20 rounded-full text-lg opacity-60">×</button>
                </span>`).join('');
        }
    });
...
function setWildTarget(idx, targetId) {
    ...
    if (card) {
        ...
        diceConfig.filter(d=>d.id!=='yellow').forEach((obj, i) => items[i].classList.toggle('selected', obj.id === targetId));
    }
    updateAllDisplays(); saveGame();
}
...
function confirmDelete(index) {
    if(confirm("Delete this game?")) { games.splice(index, 1); saveGame(); showHome(); }
}

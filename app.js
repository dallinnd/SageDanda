...
function openGameActions(index) {
    ...
    overlay.innerHTML = `<div class="action-popup">
        ...
        <div class="flex justify-center gap-10">
            <button onclick="resumeGame(${index})" class="w-16 h-16 bg-green-500 rounded-2xl flex items-center justify-center text-white shadow-lg active:scale-90 transition-all">
                <svg class="w-8 h-8 ml-1" fill="currentColor" viewBox="0 0 24 24"><path d="M8 5v14l11-7z"></path></svg>
            </button>
            <button onclick="confirmDelete(${index})" class="w-16 h-16 bg-red-500 rounded-2xl flex items-center justify-center text-white shadow-lg active:scale-90 transition-all">
                <svg class="w-8 h-8" fill="none" stroke="currentColor" viewBox="0 0 24 24">
...
function renderGame() {
        ...
        <div class="scroll-area" id="game-scroll">
            <div class="sticky top-0 bg-inherit backdrop-blur-md z-50 p-5 border-b border-[var(--border-ui)] flex justify-between items-center">
                ...
                <div class="flex items-center gap-6">
                    <button onclick="changeRound(-1)" class="nav-btn-circle ${roundNum === 1 ? 'disabled' : ''}">
                    ...
                    <button onclick="changeRound(1)" class="nav-btn-circle ${roundNum === 10 ? 'disabled' : ''}">
                ...
                <div class="space-y-3">
                    ...
                    <div id="wild-section-header" class="mt-8 border-t border-[var(--border-ui)] pt-6">
                        ...
                        <div class="wild-stack" id="wild-list-container">
                            ${(rd.wild || []).map((w, idx) => renderWildCardHtml(w, idx)).join('')}
                        </div>
                    </div>
                </div>
    ...
    const newArea = document.getElementById('game-scroll');
    if (newArea) newArea.scrollTop = savedScrollTop;
    updateAllDisplays();
}

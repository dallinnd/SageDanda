...
function showSagePopup() {
    ...
    overlay.innerHTML = `<div class="w-[85%] max-w-[320px] bg-white border-4 border-yellow-500 rounded-[40px] p-8 text-center shadow-[0_0_50px_rgba(234,179,8,0.3)]">
...
function renderGame() {
    ...
    if (activeGame.currentRound > 0) {
        ...
        prevRoundInfoHtml = `<div class="animate-fadeIn"><div class="prev-round-box"><span>Prev Round Yellow Total</span><span class="text-xl">${prevYellowSum}</span></div><div class="prev-total-box"><span>Last Round Total Score</span><span class="text-xl">${prevTotalScore}</span></div></div>`;
    }

    let diceRowsHtml = '';
    if (roundNum === 1) {
        const yellowDice = diceConfig.find(d => d.id === 'yellow');
        diceRowsHtml = `<div class="animate-fadeIn">${renderDiceRow(yellowDice, roundData)}</div><div class="mt-12 text-center animate-fadeIn px-4" style="animation-delay: 0.2s"><div class="expansion-gradient text-5xl font-black uppercase tracking-tight">Expansion Pack<br>Edition</div></div>`;
    } else {
    ...
    app.innerHTML = `<div class="scroll-area" id="game-scroll">
        ...
        <div class="p-4 pb-8">${prevRoundInfoHtml}${sageSectionHtml}<div class="section-title animate-fadeIn" style="animation-delay: 0.1s"><h3>Dice Calculators</h3></div><div class="space-y-3">${diceRowsHtml}<div id="wild-section" class="wild-section-container animate-fadeIn ${roundNum < 2 ? 'hidden' : ''}" style="animation-delay: 0.4s">
        ...
        </div></div><div class="grand-total-footer animate-fadeIn" style="animation-delay: 0.5s"><span class="text-[10px] font-black uppercase opacity-50 block mb-1">Grand Total</span><span id="grand-total-box" class="text-5xl font-black">0</span></div></div>
    </div>
...
function updateAllDisplays() {
    ...
    diceConfig.forEach(d => {
        ...
        if (valEl) {
            valEl.innerHTML = vals.map((v, i) => `<span class="inline-flex items-center bg-black/10 px-5 py-3 rounded-2xl text-xl font-black shadow-sm border border-black/5 active:scale-95 transition-transform">${v} <button onclick="event.stopPropagation(); removeVal('${d.id}', ${i})" class="ml-4 w-8 h-8 flex items-center justify-center bg-black/20 rounded-full text-lg leading-none opacity-60 active:bg-red-500 active:text-white transition-colors">×</button></span>`).join('');
        }
    });

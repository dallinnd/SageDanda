...
// 2. Logic & Scoring (Moved to top for stability)
...
function renderGame() {
    ...
    const isSageDone = isSageAlreadyComplete();

    let prevInfo = '';
    if (activeGame.currentRound > 0) {
        const pr = activeGame.rounds[activeGame.currentRound - 1];
        prevInfo = `<div class="animate-fadeIn">
            <div class="prev-round-box"><span>Prev Yellow Total</span><span class="text-xl">${(pr.yellow || []).reduce((a,b)=>a+b,0)}</span></div>
            <div class="prev-total-box"><span>Last Round Score</span><span class="text-xl">${calculateRoundTotal(pr)}</span></div>
        </div>`;
    }

    let diceHtml = (roundNum === 1) 
        ? `<div class="animate-fadeIn">${renderDiceRow(diceConfig[0], roundData)}</div><div class="mt-12 text-center expansion-gradient text-5xl font-black uppercase">Expansion<br>Edition</div>`
        : diceConfig.map((d, i) => `<div class="animate-fadeIn" style="animation-delay:${i*0.05}s">${renderDiceRow(d, roundData)}</div>`).join('');

    let sageHtml = '';
    if (roundNum >= 2) {
        sageHtml = isSageDone 
            ? `<div class="mb-6 p-6 bg-yellow-500/10 border-2 border-yellow-500/30 rounded-3xl flex items-center justify-center animate-fadeIn"><span class="text-yellow-600 font-black text-xl italic underline decoration-yellow-300">★ SAGE QUEST COMPLETE ★</span></div>`
            : `<div id="sage-container" class="mb-6 p-4 bg-black/5 rounded-3xl border border-[var(--border-ui)] animate-fadeIn">
                <div class="flex justify-between items-end mb-2"><span class="text-[10px] font-black uppercase opacity-60">Sage Progress</span><span id="sage-status-text" class="text-xs font-black uppercase text-green-500">0/6 Used</span></div>
                <div class="h-4 w-full bg-black/10 rounded-full overflow-hidden"><div id="sage-progress-fill" class="h-full bg-gradient-to-r from-green-500 to-emerald-400 transition-all duration-500" style="width: 0%"></div></div>
              </div>`;
    }
    ...
    app.innerHTML = `<div class="scroll-area" id="game-scroll">
        <div class="sticky top-0 bg-inherit backdrop-blur-md z-50 p-5 border-b border-[var(--border-ui)] flex justify-between items-center">
            ...
            <div class="flex items-center gap-6"><button onclick="changeRound(-1)" class="nav-btn ${roundNum===1?'disabled':''}"><svg class="w-6 h-6" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-width="3" d="M15 19l-7-7 7-7"></path></svg></button>
            ...
            <button onclick="changeRound(1)" class="nav-btn ${roundNum===10?'disabled':''}"><svg class="w-6 h-6" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-width="3" d="M9 5l7 7-7 7"></path></svg></button></div><div class="w-10"></div>
        </div>
        <div class="p-4 pb-8">${prevInfo}${sageHtml}<div class="section-title"><h3>Dice Calculators</h3></div><div class="space-y-3">${diceHtml}
            <div id="wild-section" class="wild-section-container animate-fadeIn ${roundNum<2?'hidden':''}">
                <div class="wild-counter-inline"><span class="text-[10px] font-black uppercase opacity-60">Wild Dice Qty</span><div class="flex items-center gap-5"><button onclick="adjustWildCount(-1)" class="wild-btn-minus">-</button><span id="wild-count-num" class="font-black text-2xl">${(roundData.wild || []).length}</span><button onclick="adjustWildCount(1)" class="wild-btn-plus">+</button></div></div>
                <div class="wild-stack" id="wild-list-container">${(roundData.wild || []).map((w, idx) => renderWildCardHtml(w, idx)).join('')}</div>
            </div>
        </div><div class="grand-total-footer"><span class="text-[10px] font-black uppercase opacity-50 block mb-1">Grand Total</span><span id="grand-total-box" class="text-5xl font-black">0</span></div></div>
    </div>
...
function updateAllDisplays() {
    ...
    // Sage UI
    const sage = calculateSageProgress(rd);
    ...
    if (sageText) {
        ...
        sageText.className = `text-xs font-black uppercase ${sage.count >= 6 ? 'text-yellow-500' : 'text-green-500'}`;
    }
    if (sageFill) {
        sageFill.style.width = `${sage.percentage}%`;
        sageFill.className = `h-full transition-all duration-500 ${sage.count >= 6 ? 'bg-gradient-to-r from-amber-300 via-yellow-500 to-amber-600' : 'bg-gradient-to-r from-green-500 to-emerald-400'}`;
    }

    // Totals
    const wildBonuses = {};
    (rd.wild || []).forEach(w => { wildBonuses[w.target] = (wildBonuses[w.target] || 0) + (w.value || 0); });
    diceConfig.forEach(d => {
        const vals = rd[d.id] || [];
        let base = (vals.reduce((a,b)=>a+b,0)) + (wildBonuses[d.id] || 0);
        let score = base;
        if (d.id === 'purple') score = base * 2;
        else if (d.id === 'blue' && rd.blueHasSparkle) score = base * 2;
        ...
        if (valEl) valEl.innerHTML = vals.map((v, i) => `<span class="inline-flex items-center bg-black/10 px-5 py-3 rounded-2xl text-xl font-black border border-black/5">${v}<button onclick="event.stopPropagation(); removeVal('${d.id}', ${i})" class="ml-4 w-8 h-8 flex items-center justify-center bg-black/20 rounded-full text-lg opacity-60">×</button></span>`).join('');
    });
    document.getElementById('round-total-display').textContent = calculateRoundTotal(rd);
...
function showSagePopup() {
    ...
    overlay.innerHTML = `<div class="w-[85%] max-w-[320px] bg-white border-4 border-yellow-500 rounded-[40px] p-8 text-center shadow-2xl">
        <div class="flex flex-col items-center gap-6">
            <div class="w-24 h-24 bg-gradient-to-tr from-amber-400 to-yellow-600 rounded-full flex items-center justify-center text-white ring-4 ring-yellow-200">
                <svg class="w-14 h-14" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-width="4" d="M5 13l4 4L19 7"></path></svg>
            </div>
            <div><h2 class="text-3xl font-black text-yellow-600 tracking-tighter">✨ SAGE QUEST ✨</h2><h3 class="text-xl font-black uppercase text-slate-400">COMPLETE</h3></div>
            <p class="text-[10px] font-black uppercase tracking-widest text-slate-300">Tap to continue</p>
        </div>
    </div>`;
...
function renderDiceRow(dice, roundData) {
    ...
    const sparkle = isBlue ? `<button id="sparkle-btn" onclick="event.stopPropagation(); toggleSparkle()" class="sparkle-btn-full ${roundData.blueHasSparkle ? 'sparkle-on' : 'sparkle-off'}">${roundData.blueHasSparkle ? 'Sparkle Activated ✨' : 'Add Sparkle?'}</button>` : '';
    return `<div onclick="setActiveInput('${dice.id}')" id="row-${dice.id}" class="dice-row p-5 rounded-2xl border-l-8 border-transparent cursor-pointer">
        ...
        <div id="${dice.id}-values" class="flex flex-wrap gap-3 mt-3"></div>${sparkle}</div>`;
}
...
function adjustWildCount(delta) {
    const rd = activeGame.rounds[activeGame.currentRound];
    if (!rd.wild) rd.wild = [];
    if (rd.wild.length + delta >= 0 && rd.wild.length + delta <= 9) {
...
function changeRound(s) {
    ...
    const isSageNow = calculateSageProgress(activeGame.rounds[curIdx]).count >= 6;
    const wasSageDone = activeGame.rounds.slice(0, curIdx).some(r => calculateSageProgress(r).count >= 6);
    if (s === 1 && isSageNow && !wasSageDone) showSagePopup();
...
function saveGame() { localStorage.setItem('panda_games', JSON.stringify(games)); localStorage.setItem('panda_settings', JSON.stringify(settings)); }

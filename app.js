...
// Global check to see if Sage was completed at any point in the game
...
function renderGame() {
    ...
    // Determine if we show the Bar or the permanent Completion Badge
    let sageSectionHtml = '';
    if (roundNum >= 2) {
        if (sageGlobalStatus) {
            sageSectionHtml = `
                <div class="mb-6 p-6 bg-yellow-500/10 border-2 border-yellow-500/30 rounded-3xl flex items-center justify-center animate-fadeIn">
        ...
        if (valEl) {
            valEl.innerHTML = vals.map((v, i) => `
                <span class="inline-flex items-center bg-black/10 px-5 py-3 rounded-2xl text-xl font-black shadow-sm border border-black/5 active:scale-95 transition-transform">
                    ${v} <button onclick="event.stopPropagation(); removeVal('${d.id}', ${i})" class="ml-4 w-8 h-8 flex items-center justify-center bg-black/20 rounded-full text-lg leading-none opacity-60 active:bg-red-500 active:text-white transition-colors">×</button>
                </span>`).join('');
        }
    });
...
function changeRound(s) { 
    ...
    // Check if it was ever completed in any rounds BEFORE this one
    const completedPreviously = activeGame.rounds.slice(0, currentRoundIdx).some(r => calculateSageProgress(r).count >= 6);

    // Only show popup if it's the FIRST time the user hits 6 colors in this session
